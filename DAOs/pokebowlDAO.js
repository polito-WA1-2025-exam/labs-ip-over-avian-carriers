...
export const listOrderPokeBowls = (idOrder) => {
    const pokeBowls = [];
    let rows = db.prepare('SELECT * FROM POKEBOWLS WHERE idOrder = ?').all(idOrder);
    rows.forEach(row => {
        pokeBowls.push(new PokeBowl(row.idSize, row.base, row.qty, row.idOrder, row.id));
    });

    pokeBowls.forEach(pokeBowl => {
        rows = db.prepare('SELECT I.id, I.name FROM POKEBOWLS P, POKEBOWLS_INGREDIENTS PI, INGREDIENTS I WHERE P.id = PI.idPokeBowls AND I.id = PI.idIngredients AND P.id = ?').all(pokeBowl.pokeBowlId);
        rows.forEach(row => {
            pokeBowl.ingredients.push(new Ingredient(row.id, row.name));
        });

        rows = db.prepare('SELECT PRO.id, PRO.name FROM POKEBOWLS P, POKEBOWLS_PROTEINS PR, PROTEINS PRO WHERE P.id = PR.idPokeBowls AND PRO.id = PR.idProteins AND P.id = ?').all(pokeBowl.pokeBowlId);
        rows.forEach(row => {
...
export const addPokeBowl = (pokeBowl) => {    
    ...
    const id = stmt.run(pokeBowl.sizeId, pokeBowl.base, pokeBowl.qty, pokeBowl.orderId).lastInsertRowid;
